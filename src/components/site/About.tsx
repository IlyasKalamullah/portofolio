      ...
      <div className="grid auto-rows-[minmax(140px,auto)] gap-4 md:grid-cols-6">
        <div className="card spotlight reveal p-7 md:col-span-4 md:row-span-2">
          <p className="mb-4 font-display text-2xl font-semibold leading-snug">
            {primaryRole(profile.headline)} yang senang membangun produk digital yang <span className="text-accent">berguna</span> dan <span className="text-accent">indah</span>.
          </p>
        ...
        <div className="card spotlight reveal flex flex-col justify-between p-6 md:col-span-2" style={delay(1)}>
        ...
        <div className="card spotlight reveal flex flex-col justify-between !border-accent !bg-accent p-6 text-black md:col-span-2" style={delay(2)}>
          <BadgeCheck />
          <div>
        ...
        {stats.map(({ value, label, icon: Icon }, i) => (
          <div key={label} className="card spotlight reveal flex items-center gap-4 p-6 md:col-span-2" style={delay(i)}>
            <div className="grid h-12 w-12 place-items-center rounded-2xl bg-white/5 text-accent"><Icon size={22} /></div>
            <div>
              <p className="font-display text-4xl font-bold"><CountUp value={value} /><span className="text-accent">+</span></p>
        ...
        {topSkills.length > 0 && (
          <div className="card spotlight reveal p-6 md:col-span-6">
            ...
            <div className="flex flex-wrap gap-2">
              {topSkills.map((s, i) => (
                <span key={s.id} style={delay(i, 60, 12)} className="reveal inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-sm">
